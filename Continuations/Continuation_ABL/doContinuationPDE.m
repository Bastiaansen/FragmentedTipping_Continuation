...
options.parameterMinimum = -0.01;
options.parameterMaximum = 41;
options.parameterIncrement = +0.0001;
options.parameterMaxSteps = 10^6; % Maximum number of par. cont. steps

options.trackParameterEachNSteps = 100; % Save how many steps? (als outputted)

options.trackStability = 1; % Keep track of stability of fixed points? Time-intensive for larger problem as eigenvalues need to be computed
...
%% Saving
pars.PDESetup = PDESetup;
pars.options = options;

save('data.mat', 'pars', 'ys', 'xs', 'ps', 'us')